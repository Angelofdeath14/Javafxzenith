...
public class EventDetailsController implements Initializable {
    ...
    private void applyCompactStyles(Parent root) {
        ...
        for (javafx.scene.Node node : root.getChildrenUnmodifiable()) {
            ...
            if (node instanceof javafx.scene.control.TextArea textArea) {
            ...
            if (node instanceof javafx.scene.layout.Region region) {
                ...
                if (region instanceof javafx.scene.layout.VBox vbox) {
                    if (vbox.getSpacing() > 3) {
                        vbox.setSpacing(1);
                    }
                }
                
                if (region instanceof javafx.scene.layout.HBox hbox) {
        ...
        try {
            ...
            controller.setOnReservationComplete(() -> {
                try {
                    ...
                    event = new EvenementService().getOneById(event.getId());
                    loadEventDetails();
