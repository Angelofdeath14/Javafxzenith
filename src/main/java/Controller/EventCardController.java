...
public class EventCardController {
    ...
    private void loadEventImage() {
        try {
            ...
            if (imagePath != null && !imagePath.isEmpty()) {
                ...
                if (imagePath.startsWith("http") || imagePath.startsWith("www")) {
                    ...
                    if (file.exists()) {
                        ...
                        System.out.println("Tentative de chargement de l'image EventCard: " + uploadedImage.getAbsolutePath());
                        
                        if (uploadedImage.exists()) {
                            image = new Image(uploadedImage.toURI().toString());
                            imageLoaded = true;
                            System.out.println("✅ Image EventCard chargée avec succès depuis le répertoire XAMPP");
                        } else {
                            System.out.println("❌ Image EventCard non trouvée: " + uploadedImage.getAbsolutePath());
                        }
                    }
                }
            ...
            System.err.println("Erreur lors du chargement de l'image EventCard: " + e.getMessage());
            loadDefaultImage(event.getType());
        }
    }
